...
(function() {
  var AUTO, BASIC_INTERFACE, IMPLICIT, Logger, NEW, Promise, Request, SUB, SUBTRANSACTIONS_INTERFACE, Transaction, WRAP_INTERACE, _, __adapterID, adapters, findAdapterFeatures, fs, getAdapter, initializer, logger, ref, resolveAdapter;
  ...
  resolveAdapter = function(name, settings, id) {
    ...
    try {
      ...
      if (adapter.id == null) {
        adapter.id = id != null ? id : name + (++__adapterID);
      }
      return adapter;
    } catch (error) {
      e = error;
  ...
  initializer.client = function(id, callerName) {
    return Request.client(id, callerName).then(function(client) {
      var ref1;
      return client != null ? client : (ref1 = getAdapter(id)) != null ? ref1.getRawClient() : void 0;
    });
  };
